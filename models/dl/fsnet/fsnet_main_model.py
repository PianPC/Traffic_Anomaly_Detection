...
import sys
import os
sys.path.append(os.path.dirname(os.path.abspath(__file__)))  # 添加当前目录到模块搜索路径
FLAGS_DEFINED = False
...
class model(abs_model):
    ...
    # 修改后代码，用来退出已加载模型重启“检测”否则会因为多个模型同时加载报错
    def gen_config(self, home, data_dir, mode='train'):
        global FLAGS_DEFINED  # 加载全局变量
        flags = tf.flags
        ...
        if not FLAGS_DEFINED:
            ...
            FLAGS_DEFINED = True
            print("[FSNet] FLAGS 已定义完成。")
        else:
            print("[FSNet] FLAGS 已存在，跳过重复定义。")
...
def parse_args():
    ...
    parser.add_argument('--test_ratio', type=float, default=0.6, help='Train set ratio')
    parser.add_argument('--max_len', type=int, default=200, help='Max flow length')
    parser.add_argument('--randseed', type=int, default=128, help='randseed')
    return parser.parse_args()

def main():
    args = parse_args()
    # 2. 防止TensorFlow解析我们的参数
    import sys
    argv_backup = sys.argv
    sys.argv = [sys.argv[0]]  # 只保留脚本名
    try:
        for test_rate in [args.test_ratio]:
            print(test_rate)
